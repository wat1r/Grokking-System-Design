...
我们可以为元数据服务器引入缓存来缓存热点行数据库。使用 Memcache 进行缓存数据，访问数据库之前的数据和应用程序服务器可以快速检查缓存是否具有所需的
行。最近最少使用（LRU）可能是我们系统的合理缓存驱逐策略。在此之下
策略，我们首先丢弃最近最少查看的行。
...
CDN 是一种分布式服务器系统，可根据地理区域向用户提供 Web 内容
用户的位置、网页的来源和内容交付服务器。 看看“CDN”
我们缓存章节中的部分。
我们的服务可以将热门视频移至 CDN：
• CDN 在多个位置复制内容。 视频更接近于
用户，并且跳数更少，视频将从更友好的网络流式传输。
• CDN 机器大量使用缓存，并且大部分可以提供内存不足的视频。
不被 CDN 缓存的不太受欢迎的视频（每天 1-20 次观看）可以由我们的服务器提供
各种数据中心。

## 13.容错

我们应该使用一致性哈希在数据库服务器之间进行分配。 一致的哈希不会
不仅有助于更换死机，还有助于在服务器之间分配负载。